 ...
 * con priorità media (Task M) interrompe il task a bassa priorità (Task L) che sta tenendo bloccato un mutex.
 ...
 * poiché il Task M sta bloccando il Task L (che detiene ancora il mutex).
 *
...
void setup()
{
  ...
  Serial.println();
  Serial.println("FreeRTOS Esempio di unbounded priority inversion");

  // crea il semaforo per la l'accesso alla sezione critica in TaskL e TaskH
  lock = xSemaphoreCreateBinary();
  // il semaforo deve partire libero
  xSemaphoreGive(lock);
